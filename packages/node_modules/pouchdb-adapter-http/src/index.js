...
function readAttachmentsAsBlobOrBuffer(row) {
  ...
  Object.keys(atts).forEach(function (filename) {
    ...
    att.data = b64StringToBluffer(att.data, att.content_type);
      ...
      // Detect if response is JSON or multipart
      async function parseResponse(res) {
        const text = await res.text();
        if (text.trim().startsWith("{")) {
          return JSON.parse(text);
        } else if (text.trim().startsWith("--")) {
          return parseMultipart(text);
        } else {
          console.error("BULK GET - Unknown response format, text starts with:", text.substring(0, 50));
          throw new Error("Unknown response format");
        }
      }

    function parseMultipart(body, boundary) {
  const boundaryRegex = boundary ? new RegExp(`--${boundary}`)
    : /--[^\n\r-]+/;

  const parts = body.split(boundaryRegex).filter(p => p.includes("{"));
  const results = parts.map(p => {
    const match = p.match(/{[\s\S]*}/m);
    if (!match) return null;
    try {
        const doc = JSON.parse(match[0]);
          // Wrap in proper _bulk_get structure if not already wrapped
          if (doc.ok) {
            // Already in { ok: {...} } format from multipart
            return {
              id: doc.ok._id,
              docs: [{ ok: doc.ok }]
            };
            ...
            return {
              id: doc.id,
              docs: [{ error: doc.error }]
            };
          } else {
            // Raw document, wrap it
            return {
              id: doc._id,
              docs: [{ ok: doc }]
            };
      ...
      console.error("BULK GET - Failed to parse JSON:", match[0].slice(0, 50), e);
      return null;
    }
  }).filter(Boolean);
  return { results };
...
function HttpPouch(opts, callback) {
  ...
  api.bulkGet = coreAdapterFun('bulkGet', function (opts, callback) {
    ...
    async function doBulkGet(cb) {
      ...
      try {
        const bulkGetUrl = genDBUrl(host, '_bulk_get' + paramsToStr(params));
        const requestBody = JSON.stringify({ docs: opts.docs});
        const response = await ourFetch(bulkGetUrl, {
          method: 'POST',
          body: requestBody,
